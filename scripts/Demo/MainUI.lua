--local Soldier = require("Demo.Soldier")
...
local function makeCard(cardWidget, layer, cardId)
	...
	card.bg:setTouchEnabled(true)
	
	card.curTime = 0
	card.totalTime = 3
	card.update = function(self, diff)
...
--quick 将touch事件改造成了 tree状的分发形式了
function MainUI:ctor(sc)
	...
	--节点不要吞掉touch事件
	touchGroup:setTouchSwallowEnabled(false)
	...
	self.myteamList = {1, 2, 3}

	local leftTop = UIHelper:seekWidgetByName(w, "leftTop")
	leftTopUI(leftTop)

	local rightTop = UIHelper:seekWidgetByName(w, "rightTop")
	rightTopUI(rightTop)
	...
	local function onRestart()
		...
		CCDirector:sharedDirector():replaceScene(sc.bg)
	end

	local skillBut = UIHelper:seekWidgetByName(w, "skillBut")
	skillBut:addTouchEventListener(onRestart)
